...
class TransactionGatewayImplSpec extends Specification {
  ...
  def "transactionGateway 결과가 정상인지 확인한다"() {
    setup:
    client.findKeyMaps(_ as String) >> response

    expect:
    def result = transactionGateway.getKeyMap("orderKey")
    result == expectResult

    where:
    desc    | response                                                                                | expectResult
    "기본"    | [KeyMapDto_생성()]                                                                        | KeyMap_생성()
    "여러 주문" | [KeyMapDto_생성(), KeyMapDto_생성(orderUnitKey: "orderUnitKey2", orderNo: 2L, contrNo: 2L)] | KeyMap_생성(orderUnitKeys: [OrderUnitKey_생성(), OrderUnitKey_생성(orderUnitKey: "orderUnitKey2", buyOrderNo: 2L, contrNo: 2L)])
  }
}
