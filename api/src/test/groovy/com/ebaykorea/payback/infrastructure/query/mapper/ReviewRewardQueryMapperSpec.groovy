...
class ReviewRewardQueryMapperSpec extends Specification {
  ...
  def "reviewRewardQueryMapper가 SmileCashSaveQueueEntity 데이터 변환"() {
    expect:
    def result = mapper.mapFromSmileCashSaveApprovalEntity(request)

    result == expectResult

    where:
    desc             | request                                                              | expectResult
    "옥션 리뷰 조회"       | SmileCashSaveApprovalEntity_생성(reasonCode: "RM04Y", saveAmount: 10L) | ReviewRewardQueryResult_생성(reviewType: EventType.Review, save: true, saveAmount: 10L)
    "옥션 리뷰(프리미엄) 조회" | SmileCashSaveApprovalEntity_생성(reasonCode: "RM05Y", saveAmount: 10L) | ReviewRewardQueryResult_생성(reviewType: EventType.ReviewPremium, save: true, saveAmount: 10L)
  }
}
