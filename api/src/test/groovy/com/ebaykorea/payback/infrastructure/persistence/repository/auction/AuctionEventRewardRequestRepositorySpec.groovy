...
import static com.ebaykorea.payback.grocery.SmileCashEventGrocery.Toss_생성
...
class AuctionEventRewardRequestRepositorySpec extends Specification {
  ...
  def repository = new AuctionEventRewardRequestRepository(queueRepository, transactionRepository, reasonCodeRepository, mapper)
  ...
  def "중복요청 여부에 따라 저장 호출을 올바르게 한다"() {
    ...
    "중복요청이 있을 경우"                 | [SmileCashSaveQueueEntity_생성(bizType: 9, reasonCode: "RM02Y")] | Toss_생성()
    "BizKey 조회건이 있지만 중복요청이 아닌 경우" | [SmileCashSaveQueueEntity_생성(bizType: 9, reasonCode: "RM01Y")] | Toss_생성(requestNo: 5L)
    "BizKey 조회건이 없는 경우"           | []                                                             | EventPlatform_생성(requestNo: 5L, budgetNo: 1L)
    _________________________________________________
  ...
  def "AuctionSmileCashEventRepository 저장 결과가 정상인지 확인"() {
    ...
    Toss_생성()              | EventRewardResultDto_생성(requestNo: 1L, savingNo: 1L, resultCode: -322) | _
    Toss_생성(requestNo: 5L) | EventRewardResultDto_생성(requestNo: 5L, savingNo: 2L, resultCode: 0)    | _
    EventPlatform_생성(requestNo: 5L, budgetNo: 1L) | EventRewardResultDto_생성(requestNo: 5L, savingNo: 2L, resultCode: 0) | _
  }
  ...
  def "예산 할당 번호에 따라 예산 집행이 되는지 확인"() {
    ...
    "예산 할당 번호가 있으면 예산집행 처리"      | EventPlatform_생성(budgetNo: 1L, saveAmount: 10) | 1
  }
  ...
  def "예산 집행 처리 결과가 실패 시 exception 발생 여부 확인"() {
    ...
    when:
    repository.saveWithBudget(EventPlatform_생성(budgetNo: 1L, saveAmount: 10))
