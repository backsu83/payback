...
class CashbackOrderDetailEntityMapperSpec extends Specification {
  ...
  def "CashbackOrderDetailEntityMapper가 정상적으로 변환하는지 확인"() {
    ...
    "스마일페이캐시백_N" | PayCashback_생성() | Cashback_생성(cashbackUnits: [SmilePayCashback_생성(payAmount: 1000L)])
    "스마일페이캐시백_Y" | PayCashback_생성() | Cashback_생성(cashbackUnits: [SmilePayCashback_생성(isSmilePay: true, clubAmount: 1000L, payAmount: 1000L)])
    "스마일페이캐시백_Y,클럽_Y" | PayCashback_생성(member: 회원_생성(true)) | Cashback_생성(cashbackUnits: [SmilePayCashback_생성(isSmilePay: true, clubAmount: 1000L, payAmount: 1000L)])
