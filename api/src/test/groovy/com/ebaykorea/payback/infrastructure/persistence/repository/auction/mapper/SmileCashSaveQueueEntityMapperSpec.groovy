...
class SmileCashSaveQueueEntityMapperSpec extends Specification {
  ...
  def "SmileCashEvent -> SmileCashSaveQueueEntity 매핑 테스트"() {
    ...
    "토스 - expirationDate 없을때 기본 만료일자"  | "토스-신세계 유니버스 클럽 가입" | Toss_생성(requestNo: 123L, saveAmount: 1000) | SmileCashSaveQueueEntity_생성(reasonCode: "RM02Y", reasonComment: "토스-신세계 유니버스 클럽 가입", bizType: 9, bizKey: "123", smileCashType: 2, saveAmount: 1000, expireDate: Timestamp.from(getDefaultEnableDate(PaybackInstants.now())), insertOperator: "memberKey", additionalReasonComment: "")
    "이벤트 - expirationDate 있을때 해당 만료일자" | "이벤트"               | EventPlatform_생성(saveAmount: 100, eventNo: 1L, expirationDate: PaybackInstants.truncatedDays(PaybackInstants.now(), 30), budgetNo: 1L) | SmileCashSaveQueueEntity_생성(reasonCode: "RM03Y", bizType: 9, bizKey: "1", smileCashType: 2, saveAmount: 100, expireDate: Timestamp.from(PaybackInstants.truncatedDays(PaybackInstants.now(), 30)), insertOperator: "memberKey", referenceKey: "1", reasonComment: "이벤트", additionalReasonComment: "", budgetNo: 1L, saveStatus: 3)
    "일반 상품평"                           | "일반 상품평"            | Review_생성(saveAmount: 100)                                     | SmileCashSaveQueueEntity_생성(reasonCode: "RM04Y", bizType: 9, bizKey: "1", smileCashType: 2, saveAmount: 100, expireDate: Timestamp.from(request.getExpirationDate()), insertOperator: "memberKey", referenceKey: "100", saveStatus: 3, reasonComment: "일반 상품평", additionalReasonComment: "(주문번호: 1)")
    "코어 프리미엄 상품평"                      | ""                  | Review_생성(saveAmount: 100, eventType: EventType.ReviewPremium) | SmileCashSaveQueueEntity_생성(reasonCode: "RM05Y", bizType: 9, bizKey: "1", smileCashType: 2, saveAmount: 100, expireDate: Timestamp.from(request.getExpirationDate()), insertOperator: "memberKey", referenceKey: "100", saveStatus: 3, additionalReasonComment: "(주문번호: 1)")
  }
