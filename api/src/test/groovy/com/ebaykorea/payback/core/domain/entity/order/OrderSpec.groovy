...
class OrderSpec extends Specification {
  def "Order 겁증"() {
    expect:
    def order = 주문정보
    order.getBundleDiscountMap() == 복수할인정보 as Map<String, BigDecimal>
    order.getExtraDiscountMap() == 즉시할인정보 as Map<String, BigDecimal>
    order.findItemSnapshotKeys() == 상품키
    order.isForCashback() == 캐시백주문대상여부

    where:
    desc      | 주문정보                                                                                                    | 복수할인정보                  | 즉시할인정보                  | 상품키                                      | 캐시백주문대상여부
    ...
    "즉시할인주문"  | Order_생성(extraDiscountUnits: [ExtraDiscountUnit_생성()])                                                  | [:]                     | ["orderUnitKey1": 100L] | ["itemSnapshotKey1"]                     | false
  }
}
