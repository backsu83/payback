  ...
  def "스마일카드 캐시백이 잘 생성되는지 확인"() {
    ...
    result.collect {
      [it.cashbackAmount, it.apply, it.toShopType(),
       it.t2Cashbacks.stream().collect { [it.orderNo, it.shopType, it.amount, it.basisAmount, it.smileCardType, it.apply] }]
    } == 결과.collect {
      [it.cashbackAmount, it.apply, it.toShopType(),
       it.t2Cashbacks.stream().collect { [it.orderNo, it.shopType, it.amount, it.basisAmount, it.smileCardType, it.apply] }]
    }
    ...
    "T2_값없음" | SmileCardCashback_생성(cashbackAmount: 1000L, isSmileCard: true, t2Cashbacks: [T2SmileCardCashback_생성(smileCardType: SmileCardType.T2, isT2: true)])           | _ | _
    "T2" | SmileCardCashback_생성(cashbackAmount: 1000L, isSmileCard: true, t2Cashbacks: [T2SmileCardCashback_생성(amount: 100L, smileCardType: SmileCardType.T2, isT2: true)]) | _ | _
    "T3" | SmileCardCashback_생성(cashbackAmount: 1000L, isSmileCard: true, t2Cashbacks: [T2SmileCardCashback_생성(amount: 100L, smileCardType: SmileCardType.T3, isT2: false)]) | _ | _
    _________________________________________________
    키맵 | 주문 | 결제 | 상품
