...
class SsgPointCancelProcessorMapperTest extends Specification {
  def mapper = Mappers.getMapper(SsgPointCancelProcessorMapper)


  def "SSG_POINT_취소API_리퀘스트_생성"() {

    expect:
    def result = mapper.mapToRequest(processorDto, certifier, "tokenId")
    ...
    "지마켓 취소 request" | SsgPointProcessorDto_생성(orderNo:111L,accountDate:"20230411",orgReceiptNo:"GMK00000") | SsgPointCertifier_생성(orderSiteType: OrderSiteType.Gmarket)
    "옥션 취소 reqest" | SsgPointProcessorDto_생성(orderNo:222L,accountDate:"20230411",orgReceiptNo:"AUC00000") | SsgPointCertifier_생성(orderSiteType: OrderSiteType.Auction)
    _________________________________________________
    expectResult | _ | _
  ...
  def "SSG_TARGET_DTO_생성"() {
   ...
   var processor =  SsgPointProcessorDto.builder()
    ...
    when:
    def result = mapper.mapToTarget(request.getBusiDt(), request.getRequestDate(), response, processor)
